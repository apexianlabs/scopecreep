    ...
    <div style={{minHeight:'100vh',background:'#f8fafc',fontFamily:'Inter,sans-serif'}}>
      <nav style={{background:'#fff',borderBottom:'1px solid #e2e8f0',height:56,display:'flex',alignItems:'center',padding:'0 24px',gap:16}}>
        <Link href="/dashboard" style={{display:'flex',alignItems:'center',gap:8,textDecoration:'none'}}>
          <div style={{width:26,height:26,borderRadius:6,background:'#7c3aed',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:800,color:'#fff'}}>S</div>
          <span style={{fontWeight:700,color:'#0f172a',fontSize:14}}>ScopeCreep</span>
        </Link>
        <div style={{flex:1}}/>
        {user && <Link href="/dashboard" style={{fontSize:13,color:'#64748b',textDecoration:'none'}}>Dashboard</Link>}
      </nav>
      <div style={{maxWidth:640,margin:'0 auto',padding:'40px 24px'}}>
        <h1 style={{fontSize:26,fontWeight:800,color:'#0f172a',marginBottom:6}}>Analyse scope change</h1>
        <p style={{fontSize:14,color:'#64748b',marginBottom:28}}>Paste the original scope and new request to get an instant analysis.</p>
